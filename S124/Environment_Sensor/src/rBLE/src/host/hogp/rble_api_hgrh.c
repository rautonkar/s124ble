 ...
 ****************************************************************************************
 *
 * @file	rble_api_hgrh.c
 *
 * @brief	rBLE-Host HOGP Report Host Role I/F API
 *
...
/******************************************************************************************/
/**
 *  @brief HID over GATT Report Host Role Enable Command
 *
 *  HID over GATT Report Host Role Enable @n
 *  
 *  @attention
 *  
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Enable(uint16_t conhdl, uint8_t con_type, uint8_t hids_inst_num, uint8_t bas_inst_num, RBLE_HIDS_CONTENT *hids, RBLE_DIS11_CONTENT *dis, RBLE_BAS_CONTENT *bas, RBLE_HGRH_EVENT_HANDLER call_back)
{
	...
	if (NULL != call_back) {
		...
		if (RBLE_MODE_ACTIVE != rble_mode) {
			...
			if ( NULL != RBLE_HGRH_Info.Hgrh_Handler ) {
				...
				RBLE_HGRH_Info.conhdl      = conhdl;
				RBLE_HGRH_Info.Hgrh_Handler = call_back;
				
				RBLE_HGRH_Info.Hids_Temp_Memory_Top		= hids;		/* Handle attribute information of HID Service */
				RBLE_HGRH_Info.Bas_Temp_Memory_Top		= bas;		/* Handle attribute information of Battery Service */
				...
				head_pnt->opcode		= RBLE_CMD_HGP_RHOST_ENABLE;
...
/******************************************************************************************/
/**
 *  @brief HID over GATT Report Host Role Disable Command
 *
 *  HID over GATT Report Host Role Disable @n
 *  
 *  @attention
 *  
 *  @b Contents of management:
...
RBLE_STATUS RBLE_HGP_RHost_Disable(uint16_t conhdl)
{
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_DISABLE;
...
RBLE_STATUS RBLE_HGP_RHost_Read_Char(uint16_t conhdl, uint8_t inst_idx, uint8_t char_code)
{
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_READ_CHAR;
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Read_By_UUID_Char(uint16_t conhdl, uint8_t inst_idx, uint8_t char_code)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->packet_type	= RBLE_PACKET_TYPE_COMMAND;
		head_pnt->packet_length = sizeof( uint16_t ) + sizeof( uint8_t ) * 2;
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_READ_CHAR_BY_UUID;
...
/******************************************************************************************/
/**
 *  @brief Read Long Characteristic Command
 *
 *  Read Long Characteristic Infomation @n
 *  
 *  @attention
 *  
 *  @b Contents of management:
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Read_Long_Char(uint16_t conhdl, uint8_t inst_idx, uint8_t char_code)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->packet_type	= RBLE_PACKET_TYPE_COMMAND;
		head_pnt->packet_length = sizeof( uint16_t ) + sizeof( uint8_t ) * 2;
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_READ_LONG_CHAR;
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Write_Char(uint16_t conhdl, uint8_t inst_idx, uint8_t char_code, uint16_t cfg_val)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_WRITE_CHAR;
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Set_Report(uint16_t conhdl, uint8_t inst_idx, RBLE_HGP_REPORT_DESC *report)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	uint8_t								cnt;
...
#endif
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_SET_REPORT;
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Write_Protocol_Mode(uint16_t conhdl, uint8_t inst_idx, uint8_t protocol_mode_val)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->packet_type	= RBLE_PACKET_TYPE_COMMAND;
		head_pnt->packet_length = sizeof( uint16_t ) + sizeof( uint8_t ) * 2;
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_SET_PROTOCOL_MODE;
 ...
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Data_Output(uint16_t conhdl, uint8_t inst_idx, RBLE_HGP_REPORT_DESC *report)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	uint8_t								cnt;
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->packet_length += sizeof( uint8_t );
#endif
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_DATA_OUTPUT;
...
/******************************************************************************************/
/**
 *  @brief Set HID Control Point Command
 *
 *  Set HID Control Point Command @n
 *  
 *  @attention
 *  
 *  @b Contents of management:
 ...
 *  @param[in]	conhdl	            Connection Handle
 *  @param[in]	inst_idx            Service Instance Index
 *  @param[in]	control_point_val   HID Control Point Value
 *
 *  @retval		RBLE_OK				Success
 *  @retval		RBLE_STATUS_ERROR	Status Error
 */
/******************************************************************************************/
RBLE_STATUS RBLE_HGP_RHost_Write_Control_Point(uint16_t conhdl, uint8_t inst_idx, uint8_t control_point_val)
{
	RBLE_STATUS 						ret = RBLE_OK;
	RBLE_HEADER_t						*head_pnt;
	...
	if(RBLE_MODE_ACTIVE != rble_mode){
		...
		head_pnt->packet_type	= RBLE_PACKET_TYPE_COMMAND;
		head_pnt->packet_length = sizeof( uint16_t ) + sizeof( uint8_t ) * 2;
		head_pnt->opcode		= RBLE_CMD_HGP_RHOST_SET_CONTROL_POINT;
		...
		RBLE_Data_Set16( conhdl );
		RBLE_Data_Set8( inst_idx );
		RBLE_Data_Set8( control_point_val );
